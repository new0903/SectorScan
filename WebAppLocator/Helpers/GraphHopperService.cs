...
namespace WebAppLocator.Helpers
{
    public class GraphHopperService : IGraphHopperService
    {
        ...
        public GraphHopperService(IHttpClientFactory clientFactory,ILogger<GraphHopperService> logger)
        {
        ...
        public async Task<List<LocationPoint>> MatchRoadAsync(List<LocationPoint> points, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                var env = Environment.GetEnvironmentVariable("GRAPHHOPPER_USE");
                if (!string.IsNullOrEmpty(env) && !bool.Parse(env))
                {
                    return listCoord;
                }
